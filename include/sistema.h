...
class Sistema {
  ...
  public:
    ...
    //<! Funções de apoio às checagens no sistema.
    
        /*! Verifica se um dado email é compatível com algum email do sistema.
            @return false caso o email seja compatível; true caso contrário.
*       */
        bool checkEmail(const string email);

        /*! Verifica se uma dada senha é compatível com alguma senha do sistema.
            @param email email fornecido.
            @return false caso a senha seja compatível; true caso contrário.
*       */
        bool checkSenha(const string senha);

        /*! Verifica se dados, email e senha, é ambos são compatíveis com o email e senha cadastrado por algum usuário.
            @param email email fornecido para checagem.
            @param senha fornecida para checagem.
            @return id do usuário onde a senha e mail sejam compatíveis ou -1 caso os dados não sejam compatíveis com o de nenhum usuário.
*       */
        int checkLogin(const string email, const string senha);


        /*! Verifica se dado um nome é único numa lista de servidores do sistema.
            @param nome nome solicitado para criação do servidor.
            @return true se já houver um nome; falso caso seja único.
        */
        bool checkServerNome(const string nome);


        /*! Verifica se, com base em um nome e um id, tal usuário é dono de um servidor        	
			@param nome o nome do servidor que se deseja mudar.
            @param id um id válido de um usuário logado no sistema.
            @return índice do servidor no sistema caso o usuário seja, de fato, o dono do servidor .
            @return -1 caso o usuário não seja seja o dono do servidor.
            @return -2 caso o servidor solicitado não existe(não tenha o nome dado).
        */
